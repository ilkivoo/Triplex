...
int main()
{
    ...
    getTotalRel("../UnpairedSitesMEG3WithBkg/triplexUnpairedSitesMEG3andBkg.tpx.summary", triplexWithBkg, keysOfTriplexWithBkg, valuesOfTriplexWithBkg);

    unordered_map<string, double> triplexWithPeaks;
    vector<string> keysOfTriplexWithPeaks;
    set<double> valuesOfTriplexWithPeaks;
    getTotalRel("../UnpairedSitesMEG3WithPeaks/triplexUnpairedSitesMEG3andPeaks.tpx.summary", triplexWithPeaks, keysOfTriplexWithPeaks, valuesOfTriplexWithPeaks);
    ...
    for (double triplex : all) {
        ...
        xs.push_back(((double)x) /76.0 );
        ys.push_back(y/335.0);
    }
    s = 0;
    last = 0;
